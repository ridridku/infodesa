...
where PEND.kode_kecamatan='27'
GROUP BY PEND.Kd_Desa");
		return $result;
  }
  
  
  function get_chart_belanja() {
        $query = $this->db->query("SELECT 
ref_desa.Nama_Desa as desa,
SUM(PEND.anggaran)as anggaran,
COALESCE(PEND.kredit, '0') as 'realisasi',PEND.Kd_Rincian
 FROM (SELECT 
REA1.kredit,
REA1.tahun,
ANG1.anggaran,
...
GROUP BY PEND.Kd_Desa");
     ...
     public function all_pendapatan()
	{
         $query = $this->db->query("SELECT
PEND.sub, 
ref_desa.Nama_Desa as desa,
SUM(PEND.anggaran)as anggaran,
COALESCE(SUM(PEND.kredit), '0') as 'realisasi'
 FROM (SELECT 
REA1.kredit,
REA1.tahun,
ANG1.anggaran,
